...
test.describe("Sentinel Red Team Gauntlet", () => {
  test("Phase 3 adversarial suite with Judge report", async ({ request }) => {
    ...
    // Keep parity with backend schema (sessionId must be >= 20 chars).
    expect(
      sessionId.length,
      `Invalid RED_TEAM_SESSION_ID "${sessionId}" (must be at least 20 chars)`,
    ...
    const prePurgeRes = await request.post("/api/rag/purge", {
      data: { sessionId },
    });
    expect(prePurgeRes.status(), "Pre-test purge failed").toBe(200);
    ...
    try {
      ...
      expect(ingestRes.status(), "Ingestion failed for red-team suite").toBe(
        200,
      );
      ...
      const postPurgeRes = await request.post("/api/rag/purge", {
        data: { sessionId },
      });

      if (!postPurgeRes.ok()) {
        // Don't hide original test failures if cleanup fails.
        // eslint-disable-next-line no-console
        console.warn(
          `Post-test purge failed for ${sessionId}: ${postPurgeRes.status()}`,
        );
      }
    }
  });
